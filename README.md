...
| `--ark-key`                                    | `str`          |  — | 环境变量 `ARK_API_KEY`                | Ark 的 OCR API Key             | 优先级：命令行 `--ark-key` > 环境变量 `ARK_API_KEY` > 代码内默认值                 |
| `--ark-model`                                  | `str`          |  — | `"doubao-1-5-thinking-vision-pro-250428"` | 使用AI模型               | 必须是你的 Ark 端点已开通的模型                                                                |
